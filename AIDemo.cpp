...
int runAIDemo(const std::string &configPath, bool stress, bool smoke) {
    ...
    if (stress || smoke) config.provider="fake"; // Offline by construction.
    SceneWorld world;
    ...
    if (config.provider=="proxy") transport=std::make_unique<ai::BridgeTransport>(JevNodeExecutable,JevDecisionBridge,
        ...
        config.managedProxy ? std::filesystem::absolute(configPath).wstring() : std::wstring{});
    ...
    while (!WindowShouldClose()) {
        ...
        const auto hovered=world.worldToCell(GetScreenToWorld2D(GetMousePosition(),camera.camera()));
        camera.resize(GetScreenWidth(),GetScreenHeight());
        ...
        world.draw(camera.visibleBounds(),nullptr,&presentation);
        ...
        if (hovered && GetMouseY()>=80) presentation.drawHover(world.cellBounds(*hovered),GetTime());
        EndMode2D();
        ...
        DrawFPS(GetScreenWidth()-80,60);
        presentation.drawTooltip(world,GetMouseY()>=80?hovered:std::nullopt,paused,GetMousePosition());
        EndDrawing();
